...
<div class="sidebar-bg"></div>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li class="breadcrumb-item active">Report on PO release (Order Settlement) Status</li> 
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h3 class="page-header">Report on PO release (Order Settlement) Status<small></small></h3>
			<!-- end page-header -->
                                ...
                                $total_no_released_not=$total_no=$total_no_released=0;
                                    foreach($query_pr->result() as $row):
										...
										if($bu_query_information->num_rows()==1){
											...
											$date1_orginal = date('Y-m-d',strtotime($single_bu_query_information->Po_date));  
											$design_date = date('Y-m-d',strtotime($original_schedule));  
											$date1=date_create($date1_orginal);
											$date2=date_create($design_date);
											$diff=date_diff($date2,$date1);
											$date1_po_date = date('d-m-Y',strtotime($single_bu_query_information->Po_date));  
				...
				<div class="row">
					<div class="col-lg-4">
						<table class="table-bordered table" border="1">
							<tr>
								<th>Total PO planned</th>
								<th><?=$total_no=$query_pr->num_rows()?></th>
							</tr>
							<tr>
								<th>Total PO released</th>
								<th><?=$total_no_released?></th>
							</tr>
							<tr>
								<th>PO not released</th>
		...
		<script type="text/javascript">
		    ...
		    function drawChart() {
		      var data = google.visualization.arrayToDataTable([
		        ...
		        ['Total PO planned',     <?=$total_no?> , "#b87333"],
				['Total PO released',      <?=$total_no_released?>, "green"],
				['PO not released',  <?=$total_no_released_not?>, "red"],
