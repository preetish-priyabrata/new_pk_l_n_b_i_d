...
<div class="sidebar-bg"></div>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li class="breadcrumb-item active">Buyer Productivity Analysis (BPA)</li> 
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
                ...
                <div class="row">
                    <div class="col-md-12 col-lg-12">
                        ...
                        <table id="table1bu" class="table table-bordered" border="1">
                            ...
                            <tbody>
                                <?php 
                                    $data_buyer_price=$data_buyer_detail_comm=$data_buyer_detail_tech=$data_buyer_detail=$data_buyer = array();
                                    ...
                                    foreach($query_pr->result() as $row):
										...
										if($query_check->num_rows()==1){
                                            ...
                                            if($bu_query_information->num_rows()==1){
                                                ...
                                                if(!empty($Cost_saving) && !empty($Cost_saving)){

                                                    $data_buyer_price[]= array('buyer_id' => $buyer_name_id,'Cost_saving'=>$Cost_saving,'Order_value'=>$Order_value);
                                                }
                    ...
                    <?php
                        if(!empty($data_buyer)){
                            ...
                            foreach ($data_buyer_new as $key_buyer):
                                ...
                                //  $data_buyer_price[]= array('buyer_id' => $buyer_name_id,'Cost_saving'=>$Cost_saving,'Order_value'=>$Order_value);
                               $total_saving_per= $total_Order_value=$total_Cost_saving=0;
                                foreach ($data_buyer_price as $key_buy_comm => $value_buy) {
                                    if($data_buyer_price[$key_buy_comm]['buyer_id']==$key_buyer){
                                        $total_Cost_saving=$total_Cost_saving+($data_buyer_price[$key_buy_comm]['Cost_saving']);  
                                        $total_Order_value=$total_Cost_saving+($data_buyer_price[$key_buy_comm]['Order_value']); 
                                       
                                    }  
                                }
                                if(($total_Cost_saving==0) && ($total_Order_value==0)){
                                    $total_saving_per=0;
                                }else{
                                    $total_saving_per=($total_Cost_saving/$total_Order_value)*100;
                                }
                                $data_buyer_graph[] = array('buyer_name' =>$key_buyer, 'total_pr_no'=>$total_pr,'total_tech_no'=>$total_pr_tech ,'total_comm_no'=>$total_pr_comm, 'total_Cost_saving' => $total_Cost_saving,'total_Order_value'=>$total_Order_value,'total_saving_per'=>$total_saving_per);
                                # code...
                            endforeach;
                            ?>
                                ...
                                <div class="col-lg-6 offset-md-1">
                                        ...
                                        <table class="table table-bordered text-center" border="1">
                                            <tr>
                                                ...
                                                <th>Actual Order Settlement No</th>
                                            </tr>
                                            <?php
                                        ...
                                        <table class="table table-bordered text-center" border="1">
                                             <tr>
                                                ...
                                                foreach ($data_buyer_graph as $key_info => $value_id) {
                                                    ...
                                                    <tr>
                                                        <td><?=$data_buyer_graph[$key_info]['buyer_name']?></td>
                                                        <td><?=$data_buyer_graph[$key_info]['total_Order_value']?></td>
                                                        <td><?=$data_buyer_graph[$key_info]['total_Cost_saving']?></td>
                                                        <td><?=$data_buyer_graph[$key_info]['total_saving_per']?></td>
                                                    </tr>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
         var data = new google.visualization.DataTable();
            data.addColumn('string', 'Buyer Name');
            data.addColumn('number', 'Pr Allocated No');
            data.addColumn('number', 'Technically Cleared No');
            data.addColumn('number', 'Actually order Settlement No');
            // data.addColumn({type: 'number', role: 'annotation'}); 
            // data.addColumn({type: 'number', role: 'annotation1'}); 
     ...
     <script type="text/javascript">
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Buyer Name', 'Spend Basic (w/o tax)', 'Saving', 'Saving %'],

                  <?php
                    foreach ($data_buyer_graph as $key_info => $value_id) :
                ?>
                ["<?=$data_buyer_graph[$key_info]['buyer_name']?> \nSaving  <?= number_format($data_buyer_graph[$key_info]['total_saving_per'],2)?>%", <?=$data_buyer_graph[$key_info]['total_Order_value']?>, <?=$data_buyer_graph[$key_info]['total_Cost_saving']?>, <?=$data_buyer_graph[$key_info]['total_saving_per']?>],
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Buyer Name',  'Saving %'],
                  ...
                  <?php
                    foreach ($data_buyer_graph as $key_info => $value_id) :
                ?>
                ['<?=$data_buyer_graph[$key_info]['buyer_name']?>', <?=$data_buyer_graph[$key_info]['total_saving_per']?>],
        ...
        var options = {
              title : 'Buyer Productivity Analysis (BPA) For Saving %',
                vAxis: {title: '% ->'},
                hAxis: {title: 'Buyer Wise -->'},
                seriesType: 'bars',
          // title : 'Monthly Coffee Production by Country',
