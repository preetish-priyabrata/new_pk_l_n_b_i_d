			                    ...
			                    <div class="form-group row pull-right">
			                        <div class="col-md-12">
			                            ...
			                            <button type="button" class="btn btn-info btn-sm m-r-5" id="upload" style="display: none">Uploading ...</button>
			                           	 ...
			                           	 if($value==1){
			                           	 	?>
			                           	 	  <a  href="<?=base_url()?>admin-view-new-category-list" class="btn btn-sm btn-danger">Cancel</a> 
			                           	 	<?php 
			                           	 }else{
			                           	 	?>
			                           	 	  <a  href="<?=base_url()?>admin-view-new-material-list" class="btn btn-sm btn-danger">Cancel</a> 
			                           	 	<?php
			                           	 }
			                           	 ?>
...
$(document).ready(function(){
	...
	$('#sub').on('click', function () {
		if (document.getElementById('job_files').value !== '') {
        	...
        	var actions_file='admin_files_uploaded_details';
            ...
            var file_data = $('#job_files').prop('files')[0];           
	            ...
	            if(file_data!=""){
	                ...
	                $.ajax({
	                    url: '<?php echo base_url(); ?>admin-excel-upload/entry', // point to server-side controller method
...
function load_data(){
    ...
    queryString_id = 'actions_file='+actions_file;
			...
			$.ajax({
				url:"<?php echo base_url(); ?>admin-excel-upload/entry",
				data:queryString_id,
				method:"POST",
