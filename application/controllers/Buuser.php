                            ...
                            foreach($objPHPExcel->getWorksheetIterator() as $worksheet){
                                ...
                                for($row=2; $row<=$highestRow; $row++){      
                                    ...
                                    $scheduled_order_DATE_st = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($scheduled_order_DATE);
                                      ...
                                      $comments = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
                                    ...
                                    if(!empty($dicipline) && !empty($pr_no) && !empty($item_name) && !empty($uom) &&   !empty($qty) && !empty($orginal_schedule_DATE) && !empty($scheduled_order_DATE)){
                                        ...
                                        if($num==0){
                                            ...
                                            $data = array('discipline'=>$dicipline,'pr_no'=>$pr_no,'area'=>  $area,'item'=>$item_name,'UOM'=> $uom,'quantity'=>$qty,'original_schedule'=>$orginal_schedule,'job_code'=>$job_code,'entry_user_id'=>$email_id,'status'=>1,'scheduled_order'=>$scheduled_order_schedule_date,'comment'=>$comments);
                                            // print_r($data);
